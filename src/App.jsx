...
import ContributionTypes from "./components/ContributionTypes";
...
function App() {
  ...
  const fetchGithubData = async () => {
    ...
    try {
      ...
      // Calculate stats for the user data
      const totalContributions = contributionData.reduce((acc, curr) => acc + curr.count, 0);
      const streaks = calculateStreaks(contributionData);
      const mostActiveDay = getMostActiveDay(contributionData);
      const averageDaily = calculateAverage(contributionData);
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-gray-900 text-white pt-6 pb-8 px-4">
        <div className="container mx-auto">
          <h1 className="text-3xl font-bold mb-2">GitHub 3D Contribution Dashboard</h1>
      ...
      <main className="container mx-auto px-4 py-8">
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                <img 
                  ...
                  className="w-24 h-24 rounded-full border-4 border-gray-100 shadow"
                />
                <div className="flex-1">
                  <h2 className="text-2xl font-bold text-gray-800 mb-1">{userData.name}</h2>
                  <p className="text-gray-600 mb-4">GitHub Contribution Dashboard</p>
                  
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-blue-50 p-3 rounded-lg border border-blue-100">
                      <p className="text-sm text-gray-500">Total Contributions</p>
                      <p className="text-xl font-semibold text-blue-600">{userData.totalContributions}</p>
                    </div>
                    <div className="bg-green-50 p-3 rounded-lg border border-green-100">
                      <p className="text-sm text-gray-500">Longest Streak</p>
                      <p className="text-xl font-semibold text-green-600">{userData.streaks.longest} days</p>
                    </div>
                    <div className="bg-purple-50 p-3 rounded-lg border border-purple-100">
                      <p className="text-sm text-gray-500">Most Active Day</p>
                      <p className="text-xl font-semibold text-purple-600">{userData.mostActiveDay}</p>
                    </div>
                    <div className="bg-amber-50 p-3 rounded-lg border border-amber-100">
                      <p className="text-sm text-gray-500">Daily Average</p>
                      <p className="text-xl font-semibold text-amber-600">{userData.averageDaily}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {!userData && !loading && (
          <div className="text-center py-16 bg-white rounded-lg shadow-md">
            ...
            <h3 className="mt-4 text-lg font-medium text-gray-900">No GitHub profile loaded</h3>
            <p className="mt-2 text-gray-500">Enter a GitHub username or try one of the examples above</p>
          </div>
        )}
      </main>
      ...
      <footer className="bg-gray-900 text-white py-6 px-4">
        <div className="container mx-auto">
          <p className="text-center text-gray-400">GitHub 3D Contribution Dashboard</p>
          <p className="text-center text-gray-500 text-sm mt-2">
            Using Three.js for 3D visualization and Recharts for statistics
          </p>
        </div>
      </footer>
    </div>
